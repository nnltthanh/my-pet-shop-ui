...
export class CartService {
  ...
  addToCart(customerId: number, cartDetail: CartDetail | null): Observable<CartDetail> {
    return this.http.post<CartDetail>(`${this.getBaseUri(customerId)}`, cartDetail);
  }
  ...
  update(customerId: number, cartDetail: CartDetail): Observable<CartDetail> {
    return this.http.put<CartDetail>(`${this.getBaseUri(customerId)}`, cartDetail);
  }

  goToVNPayPayment(paymentOption: string) {
            this.paymentByVNPay(1);

            return (this.paymentByVNPay(1));
...
paymentByVNPay(orderId: Number) {
    ...
    return this.http.post(`${environment.BACKEND_URL}/payment/${orderId}/vnpay`, data, {
        headers: {
...
// }
  ...
  getShipCost(fromDistrictId: number, fromWardCode: string, serviceId: number, toDistrictId: number, toWardCode: string) {
    fromDistrictId = 1572;
    fromWardCode = "550113";
    serviceId = 53321;
    toDistrictId = 1452;
